...
OUT_MAIN = REPORTS / "Naylor_Model_v8_Report.docx"
...
def build_appendix():
    ...
    section_intro(doc,
        what="We built three models on the same Statcast runner-season data: a per-attempt predictor "
             "(Model A), a season-level predictor (Model B, the headline), and an interpretable GLM "
    ...
    for rdat in [("Model A — per-attempt GBM", "Individual attempt", "~0.59", "Strict noise-floor test"),
                 ("Model B — season GBM", "Runner-season",
                  (f"{full_auc:.3f} (de-leaked)" if full_auc == full_auc else "0.589 (de-leaked)"),
                  "Headline predictor"),
    ...
    add_fig(doc, FIG_DIR / "Fig_v7_AUC.png", 6.0,
            "Figure A1 — Model AUC across versions; the latest bar is the de-leaked figure.")
    add_fig(doc, FIG_DIR / "Fig_v7_Importance_PrePost.png", 6.2,
            "Figure A2 — Feature importance shift after the 2023 bigger-bases rule change.")
